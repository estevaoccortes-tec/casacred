...
"""
AGENTE_08 SCR SOCIO — Extrator SCR → Excel (STRICT/BI)

- Entrada: PDFs SCR do socio, nomeados: scr_socio_[empresa]_[ano].pdf
...
def extract_cards(full_text: str) -> Dict[str, str]:
    ...
    if risco == "--":
        m4 = re.search(
            r"Classifica[c][a]o de risco:\s*\n?\s*([^\n]+)",
            full_text,
            flags=re.I,
        )
        if m4:
            hits = re.findall(r"\b([A-Z])\b", m4.group(1))
            if hits:
                risco = hits[-1].upper()

    if tot_inst == "--" or tot_ops == "--":
        m5 = re.search(
            r"Total de institui[c][o]es:.*?Total de opera[c][o]es:.*?\n\s*(\d+)\s+(\d+)",
            full_text,
            flags=re.I | re.S,
        )
        if m5:
    ...
    out["Limite de Crdito"] = limite_total
    out["Classificao de risco"] = risco
    out["Total Instituies"] = tot_inst
    out["Total Operaes"] = tot_ops
    ...
    def _extract_from_detalhamento() -> List[Dict[str, str]]:
        ...
        for ln in det_lines:
            if not re.search(r"\bModalidade\b", ln, flags=re.I) and "R$" in ln:
                ...
                if canon not in MOD_CANON and canon != "Outros cr‚ditos":
                    continue
        ...
        for canon, total in acc.items():
            out.append(
                {
                    "Modalidade": canon if canon in MOD_CANON else "Outros cr‚ditos",
                    "Valor Total (R$)": money_ptbr(f"{total:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")),
                    "Ano": meta["_ANO"],
                    "CNPJ": meta["_CNPJ"],
                    "RazÆo Social": meta["_RAZAO"],
                    "Data base (consulta)": meta["_DATA_BASE"],
                }
            )
        return out
...
def extract_linhasmodalidade_from_text(full_text: str, meta: Dict[str, str]) -> List[Dict[str, str]]:
    ...
    def _is_prod_candidate(ln: str) -> bool:
        if "R$" in ln:
            return False
        if re.search(r"\bTotal\b", ln, flags=re.I):
            return False
        ...
        if re.fullmatch(r"\d{1,3}(?:\.\d{3})*,\d{2}", ln):
            return False
        if re.fullmatch(r"Nao", ln, flags=re.I) or _no_accents_upper(ln) == "NAO":
            return False
        if re.search(r"\bA VENCER\b|\bVENCID", ln.upper()):
            return False
    ...
    for ln in lines:
        ...
        if "R$" not in ln:
            ...
            if current_produto and not pending_rows and ln[:1].islower():
                ...
                if _is_prod_candidate(current_produto):
                    last_prod_candidate = current_produto
                continue
        ...
        if "R$" in ln and re.search(r"\bA vencer\b|\bvencid|\bPreju[ií]zo\b|\bA liberar\b", ln, flags=re.I):
            ...
            if prefix_clean and _is_prod_candidate(prefix_clean):
            ...
            for faixa_raw, val_raw in pairs:
                ...
                if not current_produto and last_prod_candidate:
                    current_produto = last_prod_candidate
                row = {
    ...
    if pending_rows and last_prod_candidate:
        flush_pending_rows(last_prod_candidate)
...
def process_one_scr_pdf(
    ...
    if targets.has_detalhamento and len(linhas) == 0:
        ...
        if len(linhas) == 0:
            # Fallback: OCR all pages when detail lines are missing.
            if poppler:
                debug_dir = (outdir / (pdf_path.stem + "_debug_full")) if debug else None
                ocr_texts = ocr_pdf_pages(
                    pdf_path,
                    poppler=poppler,
                    dpi=max(dpi, 300),
                    page_numbers_1based=list(range(1, len(page_texts) + 1)),
                    debug_dir=debug_dir,
                )
                for p, txt in ocr_texts.items():
                    page_texts[p - 1] = txt
                full_text3 = "\n".join(page_texts)
                cards = extract_cards(full_text3)
                prazos = extract_prazos_from_text(full_text3, meta=cards)
                modalidades = extract_modalidades_from_text(full_text3, meta=cards)
                linhas = extract_linhasmodalidade_from_text(full_text3, meta=cards)
...
# -----------------------------
# Varredura de INPUT (scr_socio_*.pdf)
# -----------------------------
def find_scr_socio_pdfs(input_dir: Path) -> List[Path]:
    pdfs: List[Path] = []
    for root, _, files in os.walk(str(input_dir)):
        for fn in files:
            low = fn.lower()
            if low.endswith(".pdf") and "scr_socio" in low:
...
def main() -> None:
    ...
    ap.add_argument("--outdir", required=True, help=r"Pasta de saída (ex: ...\03_OUTPUT\8. SCR SOCIO)")
    ...
    pdfs = find_scr_socio_pdfs(input_dir)
    if not pdfs:
        print("ERRO: Nenhum PDF scr_socio_*.pdf encontrado no input.")
        return
