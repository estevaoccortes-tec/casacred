...
"""
AGENTE_09 SCR SACADO — Extrator SCR → Excel (STRICT/BI)

- Entrada: PDFs SCR do sacado, nomeados: scr_sacado_[empresa]_[ano].pdf
...
def extract_modalidades_from_text(full_text: str, meta: Dict[str, str]) -> List[Dict[str, str]]:
    ...
    def _extract_from_resumo(lines: List[str], card_total: float) -> List[Dict[str, str]]:
        acc: Dict[str, float] = {}
        found_outros = any(re.search(r"\boutros\b", ln, flags=re.I) for ln in lines)
        for i, ln in enumerate(lines):
            if re.search(r"\bTotal\b", ln, flags=re.I):
                continue
            prefix_norm = unicodedata.normalize("NFD", ln)
            prefix_norm = "".join(ch for ch in prefix_norm if unicodedata.category(ch) != "Mn")
            has_mod = any(rx.search(prefix_norm) for rx, _ in MOD_MATCH)
            if not has_mod:
                continue
            ...
            for j in range(i, min(i + 4, len(lines))):
                ...
                if line.strip() == "R$":
                    continue
                for m in re.findall(r"R\$\s*([\d\.\,]+)", line):
                    cand_vals.append(_to_num(m))
                for m in re.findall(r"\b\d{1,3}(?:\.\d{3})+,\d{2}\b", line):
                    cand_vals.append(_to_num(m))

            if cand_vals:
                acc[canon] = max(acc.get(canon, 0.0), max(cand_vals))

        if card_total and found_outros:
            acc_sum = sum(acc.values())
            residual = card_total - acc_sum
            if residual > 0.01 and residual / max(card_total, 1.0) <= 0.05:
                acc["Outros créditos"] = residual
    ...
    if card_total and mod_total and abs(card_total - mod_total) > 0.01:
        resumo_rows = _extract_from_resumo(lines, card_total)
        resumo_total = sum(_to_num(r.get("Valor Total (R$)", "")) for r in resumo_rows)
        if resumo_rows and abs(card_total - resumo_total) <= 0.01:
            return resumo_rows
...
def extract_linhasmodalidade_from_text(full_text: str, meta: Dict[str, str]) -> List[Dict[str, str]]:
    ...
    def _is_prod_candidate(ln: str) -> bool:
        ...
        if re.fullmatch(r"\d{1,3}(?:\.\d{3})*,\d{2}", ln):
            return False
        if re.fullmatch(r"NÆo", ln, flags=re.I) or _no_accents_upper(ln) == "NAO":
            return False
    ...
    for ln in lines:
        ...
        canon_line = canonical_modalidade(ln)
        if canon_line in MOD_CANON and (
...
# -----------------------------
# Varredura de INPUT (scr_sacado_*.pdf)
# -----------------------------
def find_scr_sacado_pdfs(input_dir: Path) -> List[Path]:
    pdfs: List[Path] = []
    for root, _, files in os.walk(str(input_dir)):
        for fn in files:
            low = fn.lower()
            if low.endswith(".pdf") and "scr_sacado" in low:
...
def main() -> None:
    ...
    ap.add_argument("--outdir", required=True, help=r"Pasta de saída (ex: ...\03_OUTPUT\11. SCR CURVA ABC)")
    ...
    pdfs = find_scr_sacado_pdfs(input_dir)
    if not pdfs:
        print("ERRO: Nenhum PDF scr_sacado_*.pdf encontrado no input.")
        return
